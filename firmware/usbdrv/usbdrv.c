 ...
 *
 * Changes:
/*		a) Do not use preinitialized global variables to avoid having to initialize
 *				the data section.
 */
#define MNHACK_NO_DATASECTION
/*		b) Interrupt-free USB handling.
 */
#define MNHACK_INT_FREE
...
#if EXPORT_USB
	USB_PUBLIC usbMsgLen_t (*_usbFunctionSetup)(uchar data[8]);
	#if USB_CFG_IMPLEMENT_FN_WRITE
		USB_PUBLIC uchar (*_usbFunctionWrite)(uchar *data, uchar len);
	#endif /* USB_CFG_IMPLEMENT_FN_WRITE */
	#if USB_CFG_IMPLEMENT_FN_READ
		USB_PUBLIC uchar (*_usbFunctionRead)(uchar *data, uchar len);
	#endif /* USB_CFG_IMPLEMENT_FN_READ */
	#if USB_CFG_IMPLEMENT_FN_WRITEOUT
		USB_PUBLIC void (*_usbFunctionWriteOut)(uchar *data, uchar len);
	#endif /* USB_CFG_IMPLEMENT_FN_WRITEOUT */
	USB_PUBLIC usbMsgLen_t (*_usbFunctionDescriptor)(struct usbRequest *rq);

	#define USBCALL(f) _ ## f
#else
	#define USBCALL(f) f
#endif
...
uchar			flags = USB_FLG_MSGPTR_IS_ROM;
		...
		return len;
#else
		return _usbFunctionDescriptor(rq);
#endif
}
 ...
 */
		...
		if(usbRxToken == (uchar)USBPID_SETUP){
				...
				if(type != USBRQ_TYPE_STANDARD){		/* standard requests are handled by driver */
						replyLen = USBCALL(usbFunctionSetup(data));
					}else{
...
#if USB_CFG_IMPLEMENT_FN_READ
				if(usbMsgFlags & USB_FLG_USE_USER_RW){
						len = USBCALL(usbFunctionRead(data, len));
				}else
#endif
				{
						uchar i = len;
...
#if EXPORT_USB
	_usbFunctionSetup = f_usbFunctionSetup;
	#if USB_CFG_IMPLEMENT_FN_WRITE
		_usbFunctionWrite = f_usbFunctionWrite;
	#endif /* USB_CFG_IMPLEMENT_FN_WRITE */
	#if USB_CFG_IMPLEMENT_FN_READ
		_usbFunctionRead = f_usbFunctionRead;
	#endif /* USB_CFG_IMPLEMENT_FN_READ */
	#if USB_CFG_IMPLEMENT_FN_WRITEOUT
		_usbFunctionWriteOut = f_usbFunctionWriteOut;
	#endif /* USB_CFG_IMPLEMENT_FN_WRITEOUT */
	_usbFunctionDescriptor = f_usbFunctionDescriptor;
#endif
